...
float * ADNS3080_DISTANCE (float DISTANCE_X, float DISTANCE_Y, float TOTAL_X, float TOTAL_Y, MD X)
{
    DISTANCE_X = convTwosComp(X.dx);
    DISTANCE_Y = convTwosComp(X.dy);
    TOTAL_X    = TOTAL_X + DISTANCE_X;
    TOTAL_Y    = TOTAL_Y + DISTANCE_Y;
    float final_x = TOTAL_X/157;
    float final_y = TOTAL_Y/157;
